...
const ChangeUsername = () => {
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            sessionStorage.setItem("keeper_username", newNick);

            showAlert("Username updated successfully!", "Success!", "success");
            navigate(-1);
        } catch (error) {
            setMessage({
                type: "error",
                text: "Failed to update nickname. Try again.",
            });
            setIsUpdating(false);
        }
    };
    ...
    return (
        <main className="flex items-center justify-center bg-purple/30 p-5 min-h-screen">
            <section className="w-full max-w-md p-8 mx-5 space-y-8 border bg-white shadow-btn2 relative">
                ...
                <div className="text-center pt-4">
                    <h2 className="text-3xl font-extrabold font-alegraya">
                        Change Username
                    </h2>
                    <p className="mt-2 text-gray-600">
                        How should Keeper call you?
                    </p>
                </div>
                ...
                <form onSubmit={handleUpdate} className="mt-8 space-y-6">
                    <div className="space-y-2">
                        ...
                        {message.text && (
                            <p
                                className={`text-sm font-bold mt-2 ${message.type === "success" ? "text-green-600" : "text-red-600"}`}
                            >
                                {message.text}
                            </p>
                        )}
