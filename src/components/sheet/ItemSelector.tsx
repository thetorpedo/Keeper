...
export default function ItemSelector({
  ...
  const displayedItems = selectedItemCategory === "Custom Items" 
    ? customItems 
  ...
  const handleCreateCustomItem = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newItem: Ability = {
      id: editingItemId || `item_${currentUser?.uid.substring(0,6)}_${Date.now()}`,
      role: "Custom Items",
      path: "Item", 
  ...
  const createItem = async (item: Ability, isEditingId: string | null) => {
    try {
      ...
      // Se estiver editando, deleta o antigo antes de salvar
      if (isEditingId) {
  ...
  const handleEditItem = (id: string) => {
    ...
    const newRollBlocks = itemToEdit.rollTable?.map((_, i) => Date.now() + 100 + i) || [Date.now() + 1];
  ...
  return (
    <div className="flex flex-col max-w-[90vw] w-full mx-auto gap-4">
      ...
      <div className="relative sm:w-250 min-h-110">
        ...
        <div className={`relative z-10 flex flex-row gap-4 ${selectedItemCategory === 'Custom Items' ? '' : 'px-4'} pb-4 mb-4 pt-9 -mt-4 overflow-y-clip overflow-x-auto w-full h-full`}>
          
          {/* ESTADO VAZIO (ITENS OFICIAIS) */}
          {displayedItems.length === 0 && selectedItemCategory !== "Custom Items" && (
          ...
          {selectedItemCategory === "Custom Items" && (
            <div className="flex relative flex-col items-center w-full h-full">
              <div className="flex flex-col w-full shrink-0 gap-4">
                <Dialog open={isDialogOpen} onOpenChange={(open) => {
                  ...
                  <DialogContent className="sm:max-w-xl max-h-[85vh] overflow-y-auto bg-white">
                    <form ref={formRef} key={editingItemId || "new"} onSubmit={handleCreateCustomItem} className="flex flex-col gap-4">
                      <DialogHeader>
                        <DialogTitle className="font-alegraya font-extrabold text-3xl">
                          {editingItemId ? "Editing item" : "Creating a new item"}
                        </DialogTitle>
                      </DialogHeader>
                      ...
                      <div className="grid grid-cols-10 gap-2">
                        ...
                        <div className="flex flex-col col-span-7">
                          <label className="font-alegraya-sans ml-1 font-bold lowercase text-base">Item Name</label>
                          <input 
                            name="name" 
                            required 
                            autoComplete="off"
                        ...
                        <div className="flex flex-col col-span-3 h-full">
                          <label className="font-alegraya-sans ml-1 font-bold lowercase text-base">Roll the die?</label>
                          <div className="flex w-full border border-black rounded-lg h-10.5 overflow-hidden font-alegraya-sans font-bold text-lg">
                            <button
                              ...
                              className={`flex-1 py-1 h-full cursor-pointer text-center transition-all ${
                                requiresRoll ? "bg-black text-white" : "bg-white text-gray-400 hover:bg-gray-100"
                              }`}
                            >YES</button>
                        ...
                        <div className="flex flex-row col-span-full gap-4 mt-2">
                          <div className="flex flex-col w-1/2 gap-1">
                            <label className="font-alegraya-sans ml-1 font-bold lowercase text-base">Inventory Slots</label>
                            <input 
                              name="slots" 
                              type="number" 
                              min="0"
                          ...
                          <div className="flex flex-col w-1/2 gap-1">
                            <label className="font-alegraya-sans ml-1 font-bold lowercase text-base text-gray-600">Damage (Optional)</label>
                            <input 
                              name="damage" 
                              type="number" 
                              min="0"
                        ...
                        <div className="flex col-span-full flex-col gap-1 mt-2">
                          <label className="font-alegraya-sans ml-1 font-bold lowercase text-base">Item Description</label>
                          <textarea 
                            name="description" 
                            required
                              ...
                              {rollBlocks.map((blockId, index) => (
                                <div key={blockId} className="flex gap-2 relative group items-start">
                                  <input 
                                    ...
                                    type="text" 
                                    required 
                                    defaultValue={editingItemData?.rollTable?.[index]?.value}
                                  ...
                                  <textarea 
                                    ...
                                    required 
                                    defaultValue={editingItemData?.rollTable?.[index]?.description}
                ...
                {/* AVISO DE VAZIO (FICA CENTRALIZADO COM MARGIN TOP) */}
              ...
              {/* RENDERIZAÇÃO DAS COLUNAS DOS CUSTOM ITEMS (COM pt-35) */}
                    ...
                    <div className="mt-0"> 
                      {col.map((item, index, array) => (
                        <Card
                          ...
                          onDelete={deleteItem}
                          onEdit={handleEditItem} // PASSA A FUNÇÃO AQUI!
                        />
                      ))}
                    </div>
                  </div>
          ...
          {/* RENDERIZAÇÃO DAS COLUNAS DOS ITENS OFICIAIS */}
          {selectedItemCategory !== "Custom Items" && itemColumns.map((col, colIndex) => (
            <div key={colIndex} className="flex shrink-0 flex-col max-w-70 w-full">
              <div className="mt-27">
                {col.map((item, index, array) => (
                  <Card
